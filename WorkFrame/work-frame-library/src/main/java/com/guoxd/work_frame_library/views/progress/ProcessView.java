...
public class ProcessView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		// 进度划过的角度 //绘制进度条
		progressPaint.setColor(Color.parseColor(calColor(progress / max,
				"#ffff0000", "#ff00ff00")));
		...
		while (drawDegree <= ARC_FULL_DEGREE) {
			...
			float lineStopX = lineStartX + ARC_LINE_LENGTH
					* (float) Math.sin(a);
			float lineStopY = lineStartY - ARC_LINE_LENGTH
					* (float) Math.cos(a);
		...
		canvas.drawCircle(centerX, centerY,
				(circleRadius - ARC_LINE_LENGTH) * 0.8f, textBgPaint);
		...
		canvas.drawText("分", centerX + textLen / 2 + 5, centerY - circleRadius
				/ 10 + h1 / 2 - (h1 - h11), textPaint);
		// 下一行文字
	...
	public void setProgress(final float progress) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				float oldProgress = ProcessView.this.progress;
				for (int i = 1; i <= 100; i++) {
					ProcessView.this.progress = oldProgress
							+ (progress - oldProgress) * (1.0f * i / 100);
					postInvalidate();
	...
	/**
	 * 计算渐变效果中间的某个颜色值。 仅支持 #aarrggbb 模式,例如 #ccc9c9b2
	 */
